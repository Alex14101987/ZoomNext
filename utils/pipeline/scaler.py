...
class Scaler:
    ...
    def update_grad(self):
        # Проверка на наличие inf или NaN в градиентах
        self.scaler.unscale_(self.optimizer)  # Сначала нужно размасштабировать
        for group in self.optimizer.param_groups:
            for param in group['params']:
                if param.grad is not None:
                    if torch.any(torch.isnan(param.grad)) or torch.any(torch.isinf(param.grad)):
                        print("Gradient contains NaN or Inf for parameter", param)
                        return  # Прерываем обновление, если есть NaN или Inf
