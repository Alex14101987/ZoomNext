# -*- coding: utf-8 -*-
# python test_for_video.py --config configs/vcod_finetune.py --model-name PvtV2B5_ZoomNeXt --evaluate --load-from pvtv2-b5-5frame-zoomnext.pth
import argparse, glob, os, re, shutil, time, cv2, torch, yaml
...
class VideoTestDataset(data.Dataset):
    def __init__(self, dataset_info: dict, shape: dict, num_frames: int = 1, overlap: int = 0):
        ...
        print("Инициализация VideoTestDataset...")
        ...
        print(f"Обнаружено групп изображений: {len(image_group_paths)}")
        print(f"Обнаружено групп масок: {len(mask_group_paths)}")

        self.total_data_paths = []
        for idx, (image_group_path, mask_group_path) in enumerate(zip(image_group_paths, mask_group_paths)):
            print(f"Обрабатывается группа {idx + 1}/{len(image_group_paths)}: {image_group_path}")
            ...
            print(f"Найдено валидных изображений: {len(valid_names)}")
            ...
            #     # Печатаем прогресс внутри группы
            #     if len(self.total_data_paths) % 100 == 0:
            #         print(f"Создано {len(self.total_data_paths)} клипов...")
            ...
            while i < len(valid_names):
                ...
                i += num_frames - overlap
        print(f"Общее количество клипов: {len(self.total_data_paths)}")

    def __getitem__(self, index):
        clip_info = self.total_data_paths[index]
        # print(f"Обрабатывается клип {index + 1}/{len(self.total_data_paths)}")
        ...
        for image_path, mask_path, group_name in clip_info:
            # print(f"Загружается изображение: {image_path}")
            image = io.read_color_array(image_path)
            mask_paths.append(mask_path)
            images = ops.ms_resize(image, scales=(0.5, 1.0, 2.0), base_h=base_h, base_w=base_w)
...
class Evaluator:
    ...
    def eval(self, model, data_loader, save_path=""):
        ...
        for batch in data_loader:
            ...
            probs = probs.numpy()
            # print(type(probs), probs.shape, end='\r')
...
def test(model, cfg):
    ...
    for te_name in cfg.test.data.names:
        ...
        te_dataset = VideoTestDataset(dataset_info=te_info, shape=cfg.test.data.shape, num_frames=cfg.num_frames, overlap=0)
